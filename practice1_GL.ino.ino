...
void setup()
{
  ...
  Timer1.setFrequency(5); // частота в Герцах
  Timer1.enableISR(); // вкл прерывания
  Timer2.setFrequency(5); // частота в Герцах
  Timer2.enableISR(); // вкл прерывания
}
...
void r_direction() {
  lcd.setCursor(15, 1);
  (motorSpeed < 0) ? lcd.write((uint8_t)1) : lcd.write((uint8_t)0);
}
